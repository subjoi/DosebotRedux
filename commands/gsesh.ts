...
export async function legacy(client: Discord.Client, message: Discord.Message, args: string[]) {
    let response = await message.reply("Group session is now a slash command! Type /gsesh.");
...
export async function performInteraction(interaction: Discord.CommandInteraction) {
    ...
    const firstMessageText = `Time for a group session. Drugs are up in two minutes! Use the reaction button to join in.`;
    ...
    const secondMessage = await interaction.channel.send("Drugs are up in one minute! Use the reaction button to join in.");
    const secondReaction = await secondMessage.react(emoji[1]);

    await new Promise((resolve) => { setTimeout(resolve, 30 * 1000) });

    const thirdMessage = await interaction.channel.send("Drugs up in 30 seconds! Use the reaction button to join in.");
    ...
    await interaction.channel.send(`Use safely ${mentionString}!`);
